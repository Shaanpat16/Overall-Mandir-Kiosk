        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--s-24)' }}>
          {CHARITIES.map((charity, i) => (
            <motion.div
              ...
              initial={{ opacity: 0, y: 24 }}
              animate={{ opacity: 1, y: 0 }}
              transition={{ duration: 0.5, ease: appleEase, delay: 0.12 + i * 0.08 }}
            >
              <motion.div
                style={{
                ...
                whileTap={{ scale: 0.98 }}
                onClick={() => setExpanded(expanded === charity.id ? null : charity.id)}
              >
                <img
                  src={charity.image}
                  alt={charity.title}
                ...
                <div style={{
                  position: 'absolute',
                  inset: 0,
                  background: 'linear-gradient(transparent 40%, rgba(28,25,22,0.6))',
                  display: 'flex',
                  alignItems: 'center',
                ...
                {expanded === charity.id && (
                  ...
                  >
                    <div style={{
                      ...
                      <p className="text-body" style={{ fontSize: 18 }}>{charity.description}</p>
                    </div>
                  </motion.div>
                )}
              </AnimatePresence>
