...
let showMapBtn = null;
let showMahantMapBtn = null;
let mahantLocationModal = null;
...
const LANGUAGES = {
    en: {
        ...
        // Buttons
        show_map: "Show Map",
        show_mahant: "Show Mahant Location",
...
function updateAllText() {
    ...
    if (showMapBtn) showMapBtn.textContent = lang.show_map;
    
    const showMahantBtn = document.getElementById('show-mahant-map');
    if (showMahantBtn) showMahantBtn.textContent = lang.show_mahant;
...
function initializeDOMElements() {
    ...
    showMapBtn = document.getElementById('show-map');
    showMahantMapBtn = document.getElementById('show-mahant-map');
    mahantLocationModal = document.getElementById('mahant-location-modal');
...
function initializeMap() {
    ...
    if (showMapBtn) {
        console.log('Setting up map button:', showMapBtn);
        
        // Remove any existing listeners to prevent duplicates
        showMapBtn.replaceWith(showMapBtn.cloneNode(true));
        const newMapBtn = document.getElementById('show-map');
        
        // Add enhanced event listeners
        addEnhancedEventListener(newMapBtn, 'click', (e) => {
            ...
            try {
                ...
                console.error('Error showing map modal:', error);
            }
        });
        
        // Add touch support
        addEnhancedEventListener(newMapBtn, 'touchstart', (e) => {
        ...
        // Add visual feedback
        addEnhancedEventListener(newMapBtn, 'mousedown', () => {
            newMapBtn.style.transform = 'scale(0.95)';
        });
        
        addEnhancedEventListener(newMapBtn, 'mouseup', () => {
            newMapBtn.style.transform = 'scale(1)';
        });
        
        addEnhancedEventListener(newMapBtn, 'mouseleave', () => {
            newMapBtn.style.transform = 'scale(1)';
        });
    } else {
...
function showMapModal() {
    ...
    mapModal.innerHTML = `
        <div class="map-modal-content">
            <div class="map-modal-header">
                <h3><i class="fas fa-map-marked-alt"></i> Mandir Location</h3>
            ...
            <div class="map-modal-body">
                <div class="map-placeholder-large">
                    ...
                    <h4>BAPS Shri Swaminarayan Mandir</h4>
                    <p>2500 Woodbridge Avenue</p>
                    <p>Edison, NJ 08817</p>
                    <div class="map-info">
                        <p><i class="fas fa-phone"></i> (732) 572 1234</p>
                        <p><i class="fas fa-envelope"></i> info.edison@usa.baps.org</p>
                    </div>
                    <div class="map-actions">
                        <button class="directions-btn" onclick="showDirections()">Show Directions</button>
    ...
    mapModal.querySelector('.map-modal-content').style.cssText = `
        background: white;
        border-radius: 20px;
        width: 100%;
        max-width: 600px;
    ...
    mapModal.querySelector('.map-modal-header').style.cssText = `
    ...
    mapModal.querySelector('.map-modal-body').style.cssText = `
        padding: 40px;
        text-align: center;
    `;
...
function showDirections() {
    ...
    directionsModal.className = 'directions-modal';
    directionsModal.innerHTML = `
        <div class="directions-content">
            <div class="directions-header">
                <h3><i class="fas fa-route"></i> Directions to BAPS Mandir</h3>
                <button class="close-btn" onclick="closeDirections()">
                    <i class="fas fa-times"></i>
                </button>
            </div>
            <div class="directions-body">
                <div class="direction-step">
                    ...
                    <p>Take I-95 South to Exit 10 (Route 1 South). Continue on Route 1 South to Woodbridge Avenue exit. Turn left onto Woodbridge Avenue.</p>
                </div>
                <div class="direction-step">
                    <h4>From New Jersey Turnpike:</h4>
                    <p>Take Exit 10 (Route 1 South). Continue on Route 1 South to Woodbridge Avenue exit. Turn left onto Woodbridge Avenue.</p>
                </div>
                <div class="direction-step">
                    <h4>From Route 287:</h4>
                    <p>Take Exit 4 (Route 1 North). Continue on Route 1 North to Woodbridge Avenue exit. Turn right onto Woodbridge Avenue.</p>
                </div>
                <p class="mandir-address">Mandir Address: 2500 Woodbridge Avenue, Edison, NJ 08817</p>
            </div>
        </div>
    `;
...
// Mahant Swami Location functionality
function showMahantModal() {
    if (mahantLocationModal) {
        mahantLocationModal.style.display = 'block';
        
        // Update modal content with current data
        updateMahantModalContent();
        
        // Add escape key listener
        document.addEventListener('keydown', handleMahantModalEscape);
    }
}

function closeMahantModal() {
    if (mahantLocationModal) {
        mahantLocationModal.style.display = 'none';
        
        // Remove escape key listener
        document.removeEventListener('keydown', handleMahantModalEscape);
    }
}

function handleMahantModalEscape(e) {
    if (e.key === 'Escape') {
        closeMahantModal();
    }
}

function updateMahantModalContent() {
    // Get current Mahant Swami data from localStorage or use defaults
    const mahantData = JSON.parse(localStorage.getItem('mahantData')) || {
        location: 'BAPS Shri Swaminarayan Mandir',
        address: 'Edison, NJ, USA',
        status: 'Available for Darshan'
    };
    
    // Update main kiosk display
    const currentLocationElement = document.getElementById('mahant-current-location');
    const currentStatusElement = document.getElementById('mahant-current-status');
    
    if (currentLocationElement) {
        currentLocationElement.textContent = `Currently at: ${mahantData.location}`;
    }
    if (currentStatusElement) {
        currentStatusElement.textContent = `Status: ${mahantData.status}`;
    }
    ...
    if (modalLocationName) modalLocationName.textContent = mahantData.location;
    if (modalLocationAddress) modalLocationAddress.textContent = mahantData.address;
    if (modalLocationStatus) modalLocationStatus.textContent = mahantData.status;
    if (mapLocationText) mapLocationText.textContent = mahantData.location;
    if (mapAddressText) mapAddressText.textContent = mahantData.address;
}

// Function to update main kiosk display with latest Mahant Swami data
function updateMainKioskMahantDisplay() {
    updateMahantModalContent();
}
    ...
    try {
        ...
        if (savedData) {
            ...
            if (kioskData.websiteImages) {
                ...
                // Load murti darshan image
                if (kioskData.websiteImages.murtiDarshan) {
                    loadMurtiDarshanImage(kioskData.websiteImages.murtiDarshan);
                }
            }
            if (kioskData.mahantLocation) {
                updateMahantLocationData(kioskData.mahantLocation);
            }
        } else {
            ...
            if (websiteImagesData.murtiDarshan) {
                loadMurtiDarshanImage(websiteImagesData.murtiDarshan);
            } else if (kioskImagesData.murtiDarshan) {
                loadMurtiDarshanImage(kioskImagesData.murtiDarshan);
            }
...
function loadHeroSliderImages(images) {
    ...
    images.forEach((imageData, index) => {
        ...
        const imageUrl = typeof imageData === 'object' && imageData.url ? imageData.url : imageData;
        const imageTitle = typeof imageData === 'object' && imageData.title ? imageData.title : `Image ${index + 1}`;
...
// Function to load murti darshan image
function loadMurtiDarshanImage(imageData) {
    console.log('🖼️ Loading murti darshan image:', imageData);
    
    if (!imageData) {
        console.log('⚠️ No murti darshan image to load');
        return;
    }
    ...
    const murtiImage = document.querySelector('.murti-section img');
    if (murtiImage) {
        murtiImage.src = imageUrl;
        murtiImage.onerror = function() {
            this.src = 'data:image/svg+xml;base64,';
        };
        console.log('✅ Murti darshan image loaded successfully');
    } else {
        console.error('❌ Murti darshan image element not found');
    }
}
...
function reinitializeEventListeners() {
    ...
    try {
        ...
        // Reinitialize Mahant location functionality
        initializeMahantLocation();
...
function updateWebsiteImages(websiteImagesData) {
    ...
    if (websiteImagesData.murtiDarshan) {
        console.log('🙏 Updating murti darshan image');
        updateMurtiDarshanImage(websiteImagesData.murtiDarshan);
    } else {
        console.log('⚠️ No murti darshan image found in website images data');
    }
...
// Update murti darshan image
function updateMurtiDarshanImage(murtiImageData) {
    if (!murtiImageData) {
        console.log('⚠️ No murti darshan image to update');
        return;
    }
    
    console.log('🖼️ Updating murti darshan image:', murtiImageData);
    
    const murtiImage = document.querySelector('.murti-container img');
    if (murtiImage) {
        murtiImage.src = murtiImageData;
        murtiImage.alt = 'Murti Darshan';
        
        // Add error handling
        murtiImage.onerror = function() {
            console.warn('⚠️ Failed to load murti darshan image:', murtiImageData);
            this.src = 'data:image/svg+xml;base64,';
        };
        
        // Add success handling
        murtiImage.onload = function() {
            console.log('✅ Successfully loaded murti darshan image');
        };
        
        console.log('✅ Murti darshan image updated successfully');
    } else {
        console.warn('⚠️ Murti darshan image container not found');
    }
}

// Update Mahant location data
function updateMahantLocationData(mahantData) {
    if (!mahantData) {
        console.log('⚠️ No Mahant location data to update');
        return;
    }
    
    console.log('🙏 Updating Mahant location data:', mahantData);
    ...
    if (currentLocationElement) {
        currentLocationElement.textContent = mahantData.location || 'BAPS Shri Swaminarayan Mandir, Edison, NJ';
        console.log('✅ Updated Mahant current location');
    } else {
        console.warn('⚠️ Mahant current location element not found');
    }
    
    if (currentStatusElement) {
        currentStatusElement.textContent = mahantData.status || 'Available for Darshan';
        console.log('✅ Updated Mahant current status');
    } else {
        console.warn('⚠️ Mahant current status element not found');
    }
    
    // Store in localStorage for modal access
    localStorage.setItem('mahantData', JSON.stringify(mahantData));
    
    console.log('✅ Mahant location data updated successfully');
}

function initializeMahantLocation() {
    console.log('🙏 Initializing Mahant Swami location functionality...');
    
    // Add event listener for the Mahant Swami map button
    if (showMahantMapBtn) {
        console.log('Setting up Mahant map button:', showMahantMapBtn);
        
        // Remove any existing listeners to prevent duplicates
        showMahantMapBtn.replaceWith(showMahantMapBtn.cloneNode(true));
        const newMahantMapBtn = document.getElementById('show-mahant-map');
        
        // Add enhanced event listeners
        addEnhancedEventListener(newMahantMapBtn, 'click', (e) => {
            console.log('🙏 Mahant map button clicked');
            e.preventDefault();
            e.stopPropagation();
            ...
            try {
                ...
                console.error('Error showing Mahant modal:', error);
            }
        });
        
        // Add touch support
        addEnhancedEventListener(newMahantMapBtn, 'touchstart', (e) => {
            console.log('👆 Mahant map button touched');
            e.preventDefault();
            newMahantMapBtn.click();
        });
        
        // Add visual feedback
        addEnhancedEventListener(newMahantMapBtn, 'mousedown', () => {
            newMahantMapBtn.style.transform = 'scale(0.95)';
        });
        
        addEnhancedEventListener(newMahantMapBtn, 'mouseup', () => {
            newMahantMapBtn.style.transform = 'scale(1)';
        });
        
        addEnhancedEventListener(newMahantMapBtn, 'mouseleave', () => {
            newMahantMapBtn.style.transform = 'scale(1)';
        });
    } else {
        console.warn('⚠️ Mahant map button not found');
    }
    
    // Add event listener for modal close button
    const closeMahantBtn = document.querySelector('.mahant-modal .close-modal');
    if (closeMahantBtn) {
        console.log('Setting up Mahant close button:', closeMahantBtn);
        
        addEnhancedEventListener(closeMahantBtn, 'click', (e) => {
            console.log('🙏 Mahant close button clicked');
            e.preventDefault();
            e.stopPropagation();
            ...
            try {
                ...
                console.error('Error closing Mahant modal:', error);
            }
        });
    } else {
        console.warn('⚠️ Mahant close button not found');
    }
    
    // Close modal when clicking outside
    if (mahantLocationModal) {
        console.log('Setting up Mahant modal outside click handler');
        
        addEnhancedEventListener(mahantLocationModal, 'click', (e) => {
            if (e.target === mahantLocationModal) {
                console.log('🙏 Mahant modal outside clicked');
                try {
                    closeMahantModal();
                } catch (error) {
                    console.error('Error closing Mahant modal from outside click:', error);
                }
            }
        });
    } else {
        console.warn('⚠️ Mahant modal not found');
    }
    ...
    try {
        ...
        console.error('Error updating Mahant modal content:', error);
    }
}
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    try {
        ...
        console.log('🔧 Step 9: Initializing Mahant location...');
        initializeMahantLocation();
        ...
        // Listen for localStorage changes to auto-update Mahant Swami display
        window.addEventListener('storage', (e) => {
            if (e.key === 'mahantData') {
                console.log('🔄 Mahant data updated from storage');
                updateMainKioskMahantDisplay();
            }
            if (e.key === 'kioskData') {
        ...
        console.log('🔧 Step 13: Setting up Mahant data polling...');
        // Poll for Mahant Swami data updates every 2 seconds
        setInterval(() => {
            updateMainKioskMahantDisplay();
        }, 2000);
        ...
        console.log('🔧 Step 20: Displaying upcoming dates...');
...
document.addEventListener('keydown', (e) => {
    if (e.key === 'Escape') {
        ...
        closeModal();
        closeDirections();
        closeMahantModal(); // Close Mahant Swami modal
    }
});
...
function testAllLinks() {
    ...
    // Test map buttons
    const mapButtons = document.querySelectorAll('.map-btn, .mahant-map-btn');
    ...
    if (testResults.issues.length > 0) {
        ...
        document.querySelectorAll('.feature-card, .map-btn, .mahant-map-btn, nav a, .slide, .dot').forEach(element => {
...
function testImageLoading() {
    ...
    if (savedData) {
        ...
        if (kioskData.websiteImages) {
            ...
            if (kioskData.websiteImages.murtiDarshan) {
                console.log('🙏 Murti darshan image:', kioskData.websiteImages.murtiDarshan);
                updateMurtiDarshanImage(kioskData.websiteImages.murtiDarshan);
            }
        } else {
...
function checkCurrentImageState() {
    ...
    const murtiImage = document.querySelector('.murti-container img');
    if (murtiImage) {
        console.log('🙏 Murti darshan image:', {
            src: murtiImage.src,
            alt: murtiImage.alt,
            naturalWidth: murtiImage.naturalWidth,
            naturalHeight: murtiImage.naturalHeight,
            complete: murtiImage.complete
        });
    } else {
        console.log('⚠️ Murti darshan image container not found');
    }
    ...
    try {
        ...
        if (savedData) {
            ...
            if (kioskData.websiteImages && kioskData.websiteImages.heroSlider) {
                ...
                console.log(`   - Murti darshan: ${!!kioskData.websiteImages.murtiDarshan}`);
...
function loadDefaultImages() {
    ...
    if (typeof MANDIR_CONFIG !== 'undefined' && MANDIR_CONFIG.gallery) {
        ...
        const defaultImageData = {
            ...
            murtiDarshan: MANDIR_CONFIG.gallery[0] // Use first image as murti darshan
        };
        ...
        heroSlider.innerHTML = `
            <div class="slide active">
                <div class="slide-content" style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); padding: 60px; border-radius: 20px; text-align: center; color: white;">
                    ...
                    <p style="font-size: 1.5rem; margin-bottom: 30px;">Welcome to our sacred temple</p>
...
const UPCOMING_DATES = {
    en: [
        { date: "15 Dec", event: "Bhajan Sandhya", time: "7:00 PM", description: "Evening devotional singing session" },
        { date: "20 Dec", event: "Yoga Classes", time: "8:00 AM", description: "Morning yoga and meditation session" },
        { date: "25 Dec", event: "Special Puja", time: "10:00 AM", description: "Special puja for peace and prosperity" },
        { date: "30 Dec", event: "Bhagavad Gita Class", time: "6:00 PM", description: "Weekly Bhagavad Gita study session" },
        { date: "5 Jan", event: "New Year Aarti", time: "6:00 AM", description: "Special aarti for the new year" },
        { date: "12 Jan", event: "Makar Sankranti", time: "All Day", description: "Celebration of the sun's journey" }
    ],
    gu: [
        { date: "15 ડિસે", event: "ભજન સંધ્યા", time: "7:00 PM", description: "સાંજનો ભક્તિ ગીત સત્ર" },
        { date: "20 ડિસે", event: "યોગ ક્લાસ", time: "8:00 AM", description: "સવારનો યોગ અને ધ્યાન સત્ર" },
        { date: "25 ડિસે", event: "વિશેષ પૂજા", time: "10:00 AM", description: "શાંતિ અને સમૃદ્ધિ માટે વિશેષ પૂજા" },
        { date: "30 ડિસે", event: "ભગવદ ગીતા ક્લાસ", time: "6:00 PM", description: "સાપ્તાહિક ભગવદ ગીતા અભ્યાસ" },
        { date: "5 જાન", event: "નવા વર્ષની આરતી", time: "6:00 AM", description: "નવા વર્ષ માટે વિશેષ આરતી" },
        { date: "12 જાન", event: "મકર સંક્રાંતિ", time: "દિવસભર", description: "સૂર્યની યાત્રાનો ઉત્સવ" }
    ],
    hi: [
        { date: "15 दिसं", event: "भजन संध्या", time: "7:00 PM", description: "शाम का भक्ति गीत सत्र" },
        { date: "20 दिसं", event: "योग क्लास", time: "8:00 AM", description: "सुबह का योग और ध्यान सत्र" },
        { date: "25 दिसं", event: "विशेष पूजा", time: "10:00 AM", description: "शांति और समृद्धि के लिए विशेष पूजा" },
        { date: "30 दिसं", event: "भगवद गीता क्लास", time: "6:00 PM", description: "साप्ताहिक भगवद गीता अध्ययन" },
        { date: "5 जन", event: "नए साल की आरती", time: "6:00 AM", description: "नए साल के लिए विशेष आरती" },
        { date: "12 जन", event: "मकर संक्रांति", time: "दिनभर", description: "सूर्य की यात्रा का उत्सव" }
    ]
};

// Function to display upcoming dates
function displayUpcomingDates() {
    const datesSection = document.querySelector('.murti-section');
    if (datesSection) {
        // Change the title to "Upcoming Dates" and make it clickable
        const title = datesSection.querySelector('h3');
        if (title) {
            title.innerHTML = `<i class="fas fa-calendar-alt"></i> ${getText('upcoming_dates')} <i class="fas fa-chevron-down toggle-icon"></i>`;
            title.style.cursor = 'pointer';
            title.onclick = toggleUpcomingDates;
        }
        
        // Create upcoming dates content (hidden by default)
        const datesContent = document.createElement('div');
        datesContent.className = 'upcoming-dates';
        datesContent.style.display = 'none'; // Hidden by default
        
        const currentLang = UPCOMING_DATES[currentLanguage] || UPCOMING_DATES.en;
        
        datesContent.innerHTML = `
            <div class="dates-grid">
                ${currentLang.map((event, index) => `
                    <div class="date-card" onclick="showEventDetails(${index}, '${currentLanguage}')">
        ...
        // Clear existing content and add new dates
        const existingContent = datesSection.querySelector('.murti-image, .upcoming-dates');
        if (existingContent) {
            existingContent.remove();
        }
        
        datesSection.appendChild(datesContent);
    }
}

// Function to toggle upcoming dates visibility
function toggleUpcomingDates() {
    const datesContent = document.querySelector('.upcoming-dates');
    const toggleIcon = document.querySelector('.toggle-icon');
    
    if (datesContent) {
        if (datesContent.style.display === 'none') {
            // Show dates
            datesContent.style.display = 'block';
            datesContent.style.animation = 'slideDown 0.3s ease-out';
            if (toggleIcon) {
                toggleIcon.className = 'fas fa-chevron-up toggle-icon';
            }
        } else {
            // Hide dates
            datesContent.style.animation = 'slideUp 0.3s ease-out';
            setTimeout(() => {
...
function showEventDetails(eventIndex, language) {
    const currentLang = UPCOMING_DATES[language] || UPCOMING_DATES.en;
    const event = currentLang[eventIndex];
    ...
    popup.innerHTML = `
        <div class="event-details-content">
            <div class="event-details-header">
                <h2>${event.event}</h2>
                <button class="close-event-popup" onclick="closeEventDetails()">&times;</button>
            </div>
            <div class="event-details-body">
                ...
                <div class="event-detail-row">
                    ...
                    <span><strong>Location:</strong> BAPS Shri Swaminarayan Mandir, Edison, NJ</span>
                </div>
                <div class="event-detail-row">
                    <i class="fas fa-users"></i>
                    <span><strong>Open to:</strong> All devotees and visitors</span>
                </div>
            </div>
            <div class="event-details-footer">
                <button class="event-action-btn" onclick="addToCalendar('${event.event}', '${event.date}', '${event.time}')">
                    <i class="fas fa-calendar-plus"></i> Add to Calendar
                </button>
                <button class="event-action-btn" onclick="shareEvent('${event.event}', '${event.date}', '${event.time}')">
...
// Function to add event to calendar (placeholder)
function addToCalendar(eventTitle, eventDate, eventTime) {
    // This would integrate with calendar apps in a real implementation
    const calendarText = `${eventTitle} on ${eventDate} at ${eventTime}`;
    showMessage(`Event added to calendar: ${calendarText}`, 'success');
    
    // For now, just show a success message
    console.log('📅 Adding to calendar:', calendarText);
}

// Function to share event (placeholder)
function shareEvent(eventTitle, eventDate, eventTime) {
    const shareText = `Join us for ${eventTitle} on ${eventDate} at ${eventTime} at BAPS Shri Swaminarayan Mandir, Edison, NJ`;
    ...
    if (navigator.share) {
        ...
        navigator.clipboard.writeText(shareText).then(() => {
            showMessage('Event details copied to clipboard!', 'success');
        }).catch(() => {
            showMessage('Event details copied to clipboard!', 'success');
        });
    }
...
// Function to update upcoming dates when language changes
